...
class TestHealthEndpointWithDegradedStatus:
    ...
    def test_health_exposes_degraded_readiness_when_db_unavailable(
        self, monkeypatch
    ):
            ...
            if preflight and isinstance(preflight, dict):
                ...
                if preflight.get("ran_at"):  # Only check if preflight actually ran
                    ...
                    if preflight.get("status") == "degraded":
                        assert preflight["checks"]["data_backend"]["detail"] == "database_url_is_test_placeholder"
...
class TestAskEndpointWithDegradedDataBackend:
    ...
    def test_ask_returns_degraded_service_status_when_data_backend_fails(
        self, monkeypatch
    ):
