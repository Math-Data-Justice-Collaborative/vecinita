...
def test_streaming_respects_fallback_order(fastapi_client):
    ...
    if complete and "metadata" in complete:
        ...
        assert ":" in model_used, f"model_used should be 'provider:model' format: {model_used}"
...
def test_different_providers_same_input(fastapi_client):
    ...
    with patch("src.agent.main.supabase") as mock_supabase:
        ...
        # First call
        response1 = fastapi_client.get("/ask", params={"question": "First question?"})

        # Second call
        response2 = fastapi_client.get("/ask", params={"question": "Second question?"})
