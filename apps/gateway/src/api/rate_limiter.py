...
class InMemoryRateLimiter(RateLimiterBackend):
    ...
    async def get_status(self, api_key: str) -> dict:
        ...
        return {
            ...
            "endpoint_requests": {
                ep: ep_state["requests"] for ep, ep_state in state["endpoints"].items()
            },
        }
...
def create_rate_limiter() -> RateLimiterBackend:
    ...
    if redis_url:
        try:
            redis_limiter: RateLimiterBackend = RedisRateLimiter(redis_url)
    ...
    limiter: RateLimiterBackend = InMemoryRateLimiter()
