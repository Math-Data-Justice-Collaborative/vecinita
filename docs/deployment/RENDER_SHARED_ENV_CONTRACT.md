...
**Gateway HTTP pipeline ingest (Modal workers without Postgres):** When Modal scraper workers must not hold any DSN, set on the **gateway** a shared secret **`SCRAPER_PIPELINE_INGEST_TOKEN`** and expose internal routes under **`/api/v1/internal/scraper-pipeline/*`** (see `backend/src/api/router_scraper_pipeline_ingest.py`). On Modal secret **`vecinita-scraper-env`**, set **`SCRAPER_GATEWAY_BASE_URL`** to the gateway’s public base URL (e.g. `https://your-gateway.onrender.com`) and the **same** **`SCRAPER_PIPELINE_INGEST_TOKEN`**. The scraper’s `get_db()` then uses HTTP to persist crawl/process/chunk/embed state on Render Postgres. You can omit **`MODAL_DATABASE_URL`** / **`DATABASE_URL`** on Modal for pipeline workers when this pair is set. Modal RPCs **`modal_scrape_job_get` / list / cancel** still require Postgres unless the gateway serves those routes with **`MODAL_SCRAPER_PERSIST_VIA_GATEWAY`** (recommended).
...
| 3 | **Modal** pipeline persistence | Either **`MODAL_DATABASE_URL`** (external DSN, not internal `dpg-*-a`) for legacy direct Postgres **or** **`SCRAPER_GATEWAY_BASE_URL` + `SCRAPER_PIPELINE_INGEST_TOKEN`** matching the gateway (HTTP ingest; no Modal DSN for workers). |
...
| 5 | **Gateway** `SCRAPER_PIPELINE_INGEST_TOKEN` | Set when Modal uses HTTP pipeline ingest; must match Modal’s token. |
