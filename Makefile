...
help:
	...
	@echo "  make scraper-pull                        Fetch/rebase apps/scraper-worker (fixes fast-forward pull failures)"
	@echo "  make scraper-validate-postgres           Run Postgres validation queries only (no scraping)"
	@echo "  make active-crawl [ARGS='--help']        Bounded active crawl → crawl_runs / crawl_fetch_attempts (needs DATABASE_URL; Playwright: cd apps/gateway && uv sync --extra scraping && uv run playwright install)"
...
dev-tmux:
	./scripts/run/dev-session.sh start-tmux

dev-attach:
	./scripts/run/dev-session.sh attach

dev-stop:
	./scripts/run/dev-session.sh stop
...
dev-gateway:
	cd apps/gateway && AGENT_SERVICE_URL='http://localhost:8000' EMBEDDING_SERVICE_URL='http://localhost:8001' \
...
dev-frontend:
	cd apps/chat-frontend && npm run dev -- --host 0.0.0.0 --port 5173
...
dev-data-management-api:
	...
	@test -d apps/data-management-api/apps/backend/vecinita_dm_api || \
		(echo "❌ data-management-api submodule not initialized." && \
		 echo "   Run: git submodule update --init apps/data-management-api" && exit 1)
	cd apps/gateway && PYTHONPATH=../../apps/data-management-api/apps/backend:../../apps/data-management-api/packages/service-clients:../../apps/data-management-api/packages/shared-schemas:../../apps/data-management-api/packages/shared-config uv run python -m uvicorn vecinita_dm_api.app:create_app --factory --host 0.0.0.0 --port 8005
...
data-management-api-key:
	...
	elif [ -f apps/data-management-api/.env ] && grep -q '^DATA_MANAGEMENT_API_KEY=' apps/data-management-api/.env; then \
		grep '^DATA_MANAGEMENT_API_KEY=' apps/data-management-api/.env | tail -n 1 | cut -d= -f2-; \
	elif [ -f apps/data-management-api/.env ] && grep -q '^DEV_ADMIN_BEARER_TOKEN=' apps/data-management-api/.env; then \
		grep '^DEV_ADMIN_BEARER_TOKEN=' apps/data-management-api/.env | tail -n 1 | cut -d= -f2-; \
	else \
...
branch-status:
	./scripts/run/branch-orchestrator.sh status
	./scripts/run/branch-orchestrator.sh save

branch-save:
	./scripts/run/branch-orchestrator.sh save

branch-restore:
	./scripts/run/branch-orchestrator.sh restore

branch-switch:
	@test -n "$(BRANCH)" || (echo "Usage: make branch-switch BRANCH=<name>" && exit 1)
	./scripts/run/branch-orchestrator.sh switch "$(BRANCH)"

branch-pull:
	@if [ -n "$(BRANCH)" ]; then \
		./scripts/run/branch-orchestrator.sh pull "$(BRANCH)"; \
	else \
		./scripts/run/branch-orchestrator.sh pull; \
	fi

branch-sync-main:
	./scripts/run/branch-orchestrator.sh sync-main
...
lint-backend:
	cd apps/gateway && uv run ruff check src tests
...
lint-fix-backend: format-backend
	cd apps/gateway && uv run ruff check --fix src tests

lint-frontend:
	cd apps/chat-frontend && npm run lint

lint-fix-frontend:
	cd apps/chat-frontend && npm run lint:fix
...
lint-scraper:
	cd apps/scraper-worker && make lint

lint-embedding-modal:
	cd apps/embedding-worker && make lint

lint-model-modal:
	cd apps/vllm-inference && make lint
...
typecheck-backend:
	cd apps/gateway && uv run --extra ci python -m mypy src

typecheck-frontend:
	cd apps/chat-frontend && npm run typecheck

typecheck-scraper:
	cd apps/scraper-worker && make type-check

format: format-backend format-frontend

format-backend:
	cd apps/gateway && uv run black --config pyproject.toml src tests scripts ../../scripts/dm_openapi_diff.py

format-frontend:
	cd apps/chat-frontend && npm run format:write

format-check: format-check-backend format-check-frontend

format-check-backend:
	cd apps/gateway && uv run black --check --config pyproject.toml src tests scripts ../../scripts/dm_openapi_diff.py

format-check-frontend:
	cd apps/chat-frontend && npm run format
...
# pip-audit runs in an ephemeral ``uv run --with`` env that can still report pip
# advisories independently of the locked project dependencies.
audit-backend:
	@set -e; \
	cd apps/gateway && \
...
audit-frontend:
	cd apps/chat-frontend && npm audit --audit-level=high
...
audit-fix-frontend:
	cd apps/chat-frontend && npm audit fix

audit-fix-data-management-frontend:
	cd apps/data-management-frontend && npm audit fix

check-data-management-api-layout:
	test -d apps/data-management-api/apps/backend
	test -d apps/data-management-api/packages/shared-config
...
test-embedding-modal:
	cd apps/embedding-worker && make test

test-model-modal:
	cd apps/vllm-inference && make test
...
test-backend-unit:
	cd apps/gateway && PYTHONPATH=.:../../apps/embedding-worker/src uv run pytest tests/ -m "unit and not llm" -v --tb=short && \
		PYTHONPATH=../../apps/data-management-api/packages/service-clients:../../apps/data-management-api/packages/shared-schemas:../../apps/data-management-api/packages/shared-config:../../apps/data-management-api/apps/backend \
		uv run pytest \
			../../apps/data-management-api/packages/service-clients/tests/ \
...
# Audit crawl first, then the standard streaming scraper (uploader: embeddings + ENABLE_DEEPSEEK_TAG_ENHANCEMENT / LLM tagging).
# Example: make active-crawl-pipeline ACTIVE_CRAWL_ARGS='--seeds-only --max-pages-total 5' SCRAPER_BATCH_FLAGS='--verbose'
active-crawl-pipeline:
	./scripts/run_active_crawl.sh $(ACTIVE_CRAWL_ARGS)
	./scripts/run_scraper_postgres_batch.sh --local $(SCRAPER_BATCH_FLAGS)
...
render-tests-render-suite:
	cd apps/gateway && uv run pytest tests/test_utils/test_render_env_contract.py tests/test_utils/test_service_endpoints.py tests/integration/test_service_integration_points_contract.py -q
...
render-connectivity-tests:
	...
	cd apps/gateway && uv run pytest tests/render/ -v --tb=short -m render_connectivity
...
test-schemathesis-gateway:
	cd apps/gateway && SCHEMATHESIS_HOOKS=tests.schemathesis_hooks uv run pytest tests/integration/test_api_schema_schemathesis.py -q \
...
test-schemathesis-gateway-stateful:
	cd apps/gateway && SCHEMATHESIS_HOOKS=tests.schemathesis_hooks uv run pytest \
...
test-schemathesis-agent:
	cd apps/gateway && SCHEMATHESIS_HOOKS=tests.schemathesis_hooks uv run pytest tests/integration/test_agent_api_schema_schemathesis.py -q \
		--tracecov-format=html,text \
...
test-schemathesis-cli:
	...
	set +a; \
	cd apps/gateway && bash scripts/run_schemathesis_live.sh

test-schemathesis-cli-agent:
	@set -a; \
	if [ -f .env ]; then . ./.env; fi; \
	set +a; \
	cd apps/gateway && SCHEMATHESIS_HOOKS=tests.schemathesis_hooks uv run pytest tests/live/test_live_schemathesis.py -m live -q
...
pact-verify-providers:
	cd apps/gateway && uv run pytest tests/pact/test_chat_gateway_provider_verify.py tests/pact/test_dm_api_provider_verify.py tests/pact/test_agent_provider_verify.py tests/pact/test_modal_sdk_message_pact_provider_verify.py -q && \
		PYTHONPATH=../../apps/data-management-api/packages/service-clients:../../apps/data-management-api/packages/shared-schemas:../../apps/data-management-api/packages/shared-config:../../apps/data-management-api/apps/backend \
		uv run pytest ../../apps/data-management-api/packages/service-clients/tests/pact/test_dm_service_clients_modal_message_pact_provider_verify.py -q

test-frontend-e2e:
	cd apps/chat-frontend && npm run test:e2e

docs-install:
	cd apps/docs-site && npm ci

docs-serve:
	cd apps/docs-site && npm run start
