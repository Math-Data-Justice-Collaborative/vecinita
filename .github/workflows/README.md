...
- `render-post-deploy.yml`: Runs after **Render Deploy** completes — waits for Render services to go live, then HTTP + pytest live smokes (avoids `checksPass` deadlock). See `docs/deployment/RENDER_STAGING_PROD_CICD.md`.
