...
def test_gateway_with_valid_api_key(fastapi_client, mock_auth_header):
    ...
    with patch("httpx.AsyncClient") as mock_http:
        ...
        mock_response.json = MagicMock(return_value={
            "valid": True,
            "metadata": {"rate_limit_tokens": 1000}
        })
...
def test_rate_limit_exceeded(fastapi_client, mock_auth_header):
    ...
    with patch("src.api.middleware.AuthenticationMiddleware") as mock_auth:
...
def test_auth_proxy_unreachable_fails_open(fastapi_client):
    ...
    with patch("httpx.AsyncClient") as mock_http:
        ...
        mock_client.post = AsyncMock(
            side_effect=ConnectionError("Auth proxy unreachable")
        )
...
@pytest.mark.rate_limit
@pytest.mark.parametrize("token_count, should_reject", [
...
def test_token_usage_tracking(fastapi_client, token_count, should_reject):
    ...
    with patch("src.api.middleware.RateLimitingMiddleware") as mock_rl:
        mock_middleware = MagicMock()
        
        # Simulate rate limit check
        if should_reject:
            mock_middleware.dispatch = AsyncMock(
...
def test_api_key_masked_in_response(fastapi_client, mock_auth_header):
    ...
    with patch("httpx.AsyncClient") as mock_http:
        ...
        if masked_key:
            ...
            assert mock_auth_header["Authorization"] not in str(masked_key), \
...
def test_request_time_tracking(fastapi_client):
    ...
    if request_time:
        try:
            ...
            assert 0 <= time_val < 60, \
                f"Unreasonable request time: {time_val}s"
        except ValueError:
