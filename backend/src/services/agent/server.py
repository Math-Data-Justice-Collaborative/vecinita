...
from .tools.static_response import create_static_response_tool
...
default_provider = (os.environ.get("DEFAULT_PROVIDER") or "deepseek").lower()  # Default to deepseek for cloud-only setup
...
# Helper function to check if a provider has its API key configured
def _has_provider_api_key(provider: str) -> bool:
    """Check if a provider has the necessary API key configured."""
...
# Helper function to get first available provider in fallback chain
def _get_available_provider() -> str:
    """Return the first available provider from: deepseek → gemini → grok → groq → openai → ollama."""
    for provider in ["deepseek", "gemini", "grok", "groq", "openai", "ollama"]:
        if _has_provider_api_key(provider):
            return provider
    return "deepseek"  # Prefer deepseek as default
...
try:
    ...
    # Initialize with DEFAULT_PROVIDER from env (validates API key exists)
    initial_provider = default_provider if _has_provider_api_key(default_provider) else _get_available_provider()
    
    CURRENT_SELECTION: Selection = {
        "provider": str(initial_provider),
        "model": default_model if default_model else None,
        "locked": bool(lock_model_selection_env),
    }
    
    logger.info(f"Provider validation: DEFAULT_PROVIDER={default_provider}, has_api_key={_has_provider_api_key(default_provider)}, using={initial_provider}")
    ...
    def _load_model_selection_from_file():
        try:
            ...
            if p.exists():
                ...
                # Validate that loaded provider has an API key
                prov = (data.get("provider") or CURRENT_SELECTION["provider"]).lower()
                # If loaded provider has no API key, fall back to available provider
                if not _has_provider_api_key(prov):
                    logger.warning(f"Loaded provider '{prov}' has no API key configured, using fallback")
                    prov = _get_available_provider()
    ...
    # Initialize default LLM: respect DEFAULT_PROVIDER from env, respecting loaded selection
    # Fallback chain only used if selected provider unavailable: deepseek → gemini → grok → groq → openai → ollama
    llm = None
    llm_provider = None
    llm_model = None
    ...
    # Add selected provider first (from CURRENT_SELECTION which respects DEFAULT_PROVIDER)
    selected = CURRENT_SELECTION["provider"].lower()
    if selected and _has_provider_api_key(selected):
        providers_to_try.append(selected)
    
    # Add fallback chain (skip ones already added)
    for provider in ["deepseek", "gemini", "grok", "groq", "openai", "ollama"]:
        if provider not in providers_to_try and _has_provider_api_key(provider):
            providers_to_try.append(provider)
    
    logger.info(f"Provider availability: {providers_to_try}")
    
    # Try each provider in order
    for provider_name in providers_to_try:
        try:
            if provider_name == "deepseek" and deepseek_api_key:
                logger.info("Initializing ChatOpenAI with DeepSeek (primary)...")
                ...
                logger.info("Initializing ChatVertexAI with Gemini (fallback)...")
                try:
            ...
            elif provider_name == "grok" and grok_api_key:
                logger.info("Initializing ChatOpenAI with Grok (fallback)...")
                grok_base_url = os.environ.get(
                    "GROK_BASE_URL") or "https://api.x.ai/v1"
                llm = ChatOpenAI(
                    temperature=0,
                ...
                logger.info("Initializing ChatGroq (fallback)...")
                ...
                logger.info("Initializing ChatOpenAI (fallback)...")
                ...
                logger.warning("⚠️ Initializing ChatOllama (last fallback)...")
                logger.warning("⚠️ Ensure Ollama is running at " + ollama_base_url)
    ...
    if not llm:
        raise RuntimeError(
            "No LLM provider configured. Set DEEPSEEK_API_KEY, GEMINI_API_KEY, GROK_API_KEY, "
            "GROQ_API_KEY, OPENAI_API_KEY/OPEN_API_KEY, or OLLAMA_BASE_URL.")
...
class AgentState(TypedDict):
    ...
    token_count: int  # Tracks tokens generated
    model_used: str | None  # Tracks which model was actually used
...
def _get_llm_with_tools(provider: str | None, model: str | None):
    ...
    Supported providers: 'deepseek', 'gemini', 'grok', 'groq', 'openai', 'llama'.
    Defaults to the first available in order: deepseek → gemini → grok → groq → openai → ollama.
    """
    ...
    selected_provider = (provider or CURRENT_SELECTION["provider"] or "deepseek").lower()
    
    # Validate that selected provider has API key configured
    if not _has_provider_api_key(selected_provider):
        logger.warning(f"Selected provider '{selected_provider}' has no API key, falling back to available provider")
        selected_provider = _get_available_provider()
    ...
    if selected_provider == "deepseek":
        ...
        if not gemini_api_key:
            raise RuntimeError("Gemini provider requested but GEMINI_API_KEY is not set.")
        try:
            from langchain_google_genai import ChatGoogleGenerativeAI
            use_model = model or CURRENT_SELECTION.get("model") or "gemini-pro"
            ...
            raise RuntimeError("google-genai not installed; install with: pip install google-generativeai")
    
    elif selected_provider == "grok":
        if not grok_api_key:
            raise RuntimeError("Grok provider requested but GROK_API_KEY is not set.")
        use_model = model or CURRENT_SELECTION.get("model") or "grok-beta"
        grok_llm = ChatOpenAI(
            temperature=0,
            api_key=grok_api_key,
            model=use_model,
        ...
        return grok_llm.bind_tools(tools)
    
    elif selected_provider == "groq":
        if not groq_api_key:
            raise RuntimeError("Groq provider requested but GROQ_API_KEY is not set.")
        use_model = model or CURRENT_SELECTION.get("model") or "llama-3.1-8b-instant"
        groq_llm = ChatGroq(
            temperature=0,
    ...
    elif selected_provider == "llama" or selected_provider == "ollama":
        # Prefer local Ollama, then Groq-hosted Llama
        if ollama_base_url:
            use_model = model or ollama_model or "llama3.2"
            logger.warning(f"⚠️ Using Ollama at {ollama_base_url}, ensure service is running")
            ...
            logger.info("Ollama not available, falling back to Groq-hosted Llama")
        ...
        raise RuntimeError(
            ...
            f"Use: deepseek, gemini, grok, groq, openai, or llama.")
...
def agent_node(state: AgentState) -> AgentState:
    ...
    # Extract model info for metadata tracking
    # RunnableBinding stores the original LLM in .bound attribute
    provider, model = _extract_model_info(llm_with_tools.bound)
...
class TokenStreamingCallback(BaseCallbackHandler):
    """Callback handler to track token streaming from LLM."""
    ...
    def on_llm_end(self, response, **kwargs) -> None:
        ...
        # Try to get usage info from response if available
        if hasattr(response, 'usage_metadata'):
            usage = response.usage_metadata
            if isinstance(usage, dict):
                self.token_count = usage.get('output_tokens', self.token_count)


def _extract_model_info(llm) -> tuple[str, str]:
    """Extract model name and provider from LLM instance.
    ...
    if llm_class == "ChatOpenAI":
        # Could be DeepSeek, OpenAI, or Grok (all use ChatOpenAI)
        model = getattr(llm, 'model_name', 'unknown') or getattr(llm, 'model', 'unknown')
        base_url = getattr(llm, 'api_base', '') or getattr(llm, 'base_url', '')
        
        if 'deepseek' in base_url.lower():
            return ('deepseek', model)
        elif 'x.ai' in base_url.lower():
            return ('grok', model)
        else:
            return ('openai', model)
    elif llm_class == "ChatGroq":
...
async def ask_question(
    ...
    try:
        ...
        # Note: Static FAQ checking is handled by the agent via static_response_tool.
        # The agent's system prompt prioritizes checking static responses first,
        # so no pre-filtering is needed here. This allows the agent to orchestrate
        # all tool calls (static_response_tool, db_search, web_search, etc.) intelligently.
        ...
        # Prepare state with token tracking
        initial_state = {
            "messages": messages,
            "question": question,
            "language": lang,
        ...
        # Get metadata from result state
...
def _stream_answer_tokens(answer: str, batch_size: int = 3):
    """Yield answer tokens in batches for streaming.
    
    Args:
        answer: The complete answer text
        batch_size: Number of words to batch before yielding
        
    Yields:
        Tuples of (token, full_token_text) for streaming response
    """
    words = answer.split()
...
async def ask_question_stream(
    ...
    """Enhanced streaming endpoint with token-by-token answer streaming.

    Sends JSON objects showing agent's thinking process and answer tokens:
    ...
    - {"type": "complete", "answer": "...", "sources": [...], "metadata": {...}}
    """
    # Accept both 'question' and legacy 'query' parameter names
    if question is None and query is not None:
        question = query
    ...
    async def generate_stream():
        try:
            ...
            # Note: Static FAQ checking is handled by the agent via static_response_tool.
            # The agent will stream its response including any static FAQ matches.
...
"""
            ...
            # Prepare state with new token tracking fields
            initial_state = {
                "messages": messages,
                "question": question,
                "language": lang_local,
            ...
            logger.info(
                "Invoking LangGraph agent with token streaming...")
            ...
            # Stream answer tokens to client
            cumulative_answer = ""
            for token_batch in _stream_answer_tokens(answer, batch_size=3):
                cumulative_answer += token_batch + " "
                yield f'data: {json.dumps({"type": "token", "content": token_batch, "cumulative": cumulative_answer.strip()})}\n\n'
                # Small delay to make streaming visible to client
                    ...
                    try:
                        ...
                        for doc in docs:
                            ...
                            if url and url not in seen_urls:
                                ...
                                seen_urls.add(url)
                                # Yield source as it's extracted
                                yield f'data: {json.dumps({"type": "source", "url": url, "title": source_entry["title"], "type": "document"})}\n\n'
                    except Exception:
                        pass
                    ...
                    try:
                        ...
                        for r in results:
                            ...
                            if url and url not in seen_urls:
                                ...
                                sources.append(entry)
                                # Yield source as it's extracted
                                yield f'data: {json.dumps({"type": "source", "url": url, "title": entry["title"], "type": "link"})}\n\n'
                    except Exception:
                        pass
            ...
            # Yield complete response with metadata (debugging only, not for user display)
            complete_data = {
                "type": "complete",
                "answer": answer,
                "sources": sources,
...
def config():
    ...
    # DeepSeek (primary)
    if deepseek_api_key:
        providers.append({"key": "deepseek", "label": "DeepSeek", "order": 1})
        models["deepseek"] = ["deepseek-chat", "deepseek-reasoner"]
    # Gemini (fallback)
    if gemini_api_key:
        providers.append({"key": "gemini", "label": "Gemini", "order": 2})
        models["gemini"] = ["gemini-pro", "gemini-pro-vision"]
    # Grok (fallback)
    if grok_api_key:
        providers.append({"key": "grok", "label": "Grok", "order": 3})
        models["grok"] = ["grok-beta"]
    # Groq (fallback)
    if groq_api_key:
        providers.append({"key": "groq", "label": "Groq (Llama)", "order": 4})
        models["groq"] = ["llama-3.1-8b-instant"]
    # OpenAI (fallback)
    if openai_api_key:
        providers.append({"key": "openai", "label": "OpenAI", "order": 5})
        models["openai"] = ["gpt-4o-mini", "gpt-4o", "gpt-4-turbo"]
    # Llama via Ollama (local fallback)
    if ollama_base_url:
        providers.append({"key": "llama", "label": "Llama (Local)", "order": 6})
