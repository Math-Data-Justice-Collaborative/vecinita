...
print_usage() {
  cat <<EOF
Usage: ./dev-session.sh [start|attach|stop|restart]
...
attach_session() {
  ...
  if tmux_has_session; then
    ...
    echo "No session found. Run: ./dev-session.sh start"
    exit 1
  fi
}
