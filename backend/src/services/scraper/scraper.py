...
class VecinaScraper:
    ...
    def __init__(
        ...
        try:
            ...
            log.warning("Database upload disabled. Chunks will only be written to file.")
            self.uploader = None
        ...
        log.info("VecinaScraper initialization complete")

    def scrape_urls(self, urls: list[str], force_loader: str | None = None) -> tuple[int, int, int]:
        """
    ...
    def _process_single_url(self, url: str, force_loader: str | None = None) -> None:
        """Process a single URL with detailed logging."""
        log.debug(f"[Loading] Attempting to load: {url}")

        # Load the URL
        try:
            docs, loader_type, success = self.loader.load_url(url, self.failed_log, force_loader)
            log.debug(
        ...
        if not success or not docs:
            log.warning(f"[Load Failed] Could not load {url} (loader: {loader_type})")
            self.stats["failed"] += 1
        ...
        # Process documents
        try:
            log.debug(f"[Processing] Processing {len(docs)} document(s) from {url}")
            ...
            if chunks_written > 0:
                ...
                if (
                    self.stream_mode
                    and self.uploader
                    and getattr(self.processor, "last_chunks", None)
                ):
                    log.debug(
                    ...
                    try:
                        ...
                        log.debug(f"[Upload Result] {uploaded} uploaded, {failed} failed")
                    except Exception as upload_err:
                        log.error(f"[Upload Error] Failed to upload chunks: {upload_err}")
                        self.stats["failed_uploads"] += len(self.processor.last_chunks)

                # Track links
                if extracted_links:
                    self.link_tracker.add_links(url, extracted_links, loader_type)
                    self.stats["total_links"] += len(extracted_links)
                    log.debug(f"[Links] Tracked {len(extracted_links)} links from {url}")
                    ...
                    try:
                        ...
                        if pw_success and pw_docs:
                            ...
                            if chunks_written > 0:
                                ...
                                if extracted_links:
                                    self.link_tracker.add_links(url, extracted_links, pw_loader)
                                    self.stats["total_links"] += len(extracted_links)
                                    log.debug(
                        ...
                        self.stats["failed"] += 1
                        self.failed_sources[url] = (
                            "No usable chunks after processing (with Playwright fallback)"
                        )
                        return
                        ...
                        self.stats["failed"] += 1
                        self.failed_sources[url] = (
                            f"No usable chunks; Playwright fallback error: {e_fallback}"
                        )
                        return
                else:
                    log.warning(f"[Processing] No usable chunks generated from {url}")
                    self.stats["failed"] += 1
                    self.failed_sources[url] = "No usable chunks after processing"

        except Exception as e:
            log.error(f"[Processing Error] Exception processing {url}: {e}")
            log.debug("Full exception:", exc_info=True)
            self.stats["failed"] += 1
    ...
    def print_summary(self) -> None:
        ...
        if self.stream_mode:
            log.info(f"Chunks uploaded to database: {self.stats['total_uploads']}")
    ...
    def _upload_chunks_from_file(self) -> None:
        ...
        if not self.output_file or not self.uploader:
            log.warning("Cannot upload chunks: output_file or uploader not set")
            return

        try:
            log.info(f"Reading chunks from {self.output_file} for database upload...")
            ...
            with open(self.output_file, encoding="utf-8") as f:
                for line in f:
                    ...
                    if line.startswith("SOURCE:"):
                        ...
                        log.debug(f"Started new chunk from SOURCE: {current_source}")
                        ...
                        try:
                            ...
                            if current_chunk:
                                current_chunk["metadata"] = json.loads(metadata_str)
                            log.debug(f"Parsed METADATA: {metadata_str[:50]}...")
                        except Exception as e:
            ...
            log.info(f"Parsed {line_count} lines and extracted {chunk_count} chunks from file")

            if not chunks_data:
                log.warning("No chunks found in output file")
                return

            log.info(f"Found {len(chunks_data)} chunks in output file. Uploading to database...")
            ...
            for source_url, chunks in chunks_by_source.items():
                ...
                formatted_chunks = [
                    {"text": chunk.get("text", "").strip(), "metadata": chunk.get("metadata", {})}
                    for chunk in chunks
                    if chunk.get("text", "").strip()
                ]

                log.debug(f"Formatted {len(formatted_chunks)} chunks for {source_url}")
