const OLLAMA_CATALOG_FAMILIES = {
  families: [{ slug: "qwen2.5" }, { slug: "llama3.2" }],
};

const OLLAMA_CATALOG_TAGS: Record<
  string,
...
export const OLLAMA_MODELS_LIST_BODY = {
  items: [
    { model_id: "qwen2.5:1.5b-instruct", available: true },
...
export function mockOllamaApiFetch(
  url: string,
  ...
  if (url.includes("/internal/v1/models/ollama/catalog/")) {
    ...
    const body = OLLAMA_CATALOG_TAGS[slug] ?? { slug, tags: [] };
    return { ok: true, json: async () => body };
  }
  if (url.includes("/internal/v1/models/ollama/catalog")) {
    return { ok: true, json: async () => OLLAMA_CATALOG_FAMILIES };
  }
  if (
    url.includes("/internal/v1/models/ollama") &&
    !url.includes("/internal/v1/models/ollama/pull")
  ) {
    return { ok: true, json: async () => OLLAMA_MODELS_LIST_BODY };
  }
  return null;
}
