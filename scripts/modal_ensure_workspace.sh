...
# Prefer bare ``modal`` when on PATH (local + bug harness); else ``uv run modal`` (GHA).
if command -v modal >/dev/null 2>&1; then
  _modal() { modal "$@"; }
elif command -v uv >/dev/null 2>&1; then
  _modal() { uv run modal "$@"; }
else
  echo "modal CLI not found. Install: pip install modal (or use uv in this repo)." >&2
  exit 1
fi

_modal_token_info_ok() {
  _modal token info >/dev/null 2>&1
}
...
_wait_for_modal_token() {
  ...
  while (( attempt <= max_attempts )); do
    ...
    last_err="$(_modal token info 2>&1 || true)"
...
if [[ -n "${MODAL_TOKEN_ID:-}" && -n "${MODAL_TOKEN_SECRET:-}" ]]; then
  WORKSPACE_LINE="$(_modal token info 2>/dev/null | sed -n 's/^Workspace: //p' | head -1)"
  ...
  CURRENT="$(_modal profile current 2>/dev/null | tr -d '[:space:]')"
  if [[ "$CURRENT" != "$REQUIRED" ]]; then
    echo "==> Switching Modal profile: ${CURRENT:-unknown} -> ${REQUIRED}"
    _modal profile activate "$REQUIRED"
  fi
