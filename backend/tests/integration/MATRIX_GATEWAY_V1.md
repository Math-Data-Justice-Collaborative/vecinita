...
- `implemented`: test exists and is mapped
...
|---|---|---|---|---|---|---|
| GATEWAY-ASK-001 | GET | /api/v1/ask | API Key | Auth Proxy, Rate Limiter, Agent Service, Vector Store | Success, Auth Failure, Validation Failure, Upstream Failure, Rate Limit | planned |
| GATEWAY-ASK-002 | GET | /api/v1/ask/stream | API Key | Auth Proxy, Rate Limiter, Agent Service (SSE) | SSE Success, Auth Failure, Stream Failure, Upstream Failure, Rate Limit | planned |
| GATEWAY-ASK-003 | GET | /api/v1/ask/config | Public | Agent Service | Success, Fallback/Degraded Config | planned |
...
| GATEWAY-SCRAPE-005 | POST | /api/v1/scrape/reindex | Admin JWT | Admin JWT Verify, Modal Reindex Service | Admin Success, Non-admin Rejection, Upstream Failure | planned |
...
| GATEWAY-EMBED-001 | POST | /api/v1/embed | API Key | Auth Proxy, Rate Limiter, Embedding Service | Success, Auth Failure, Validation Failure, Upstream Failure, Rate Limit | planned |
| GATEWAY-EMBED-002 | POST | /api/v1/embed/batch | API Key | Auth Proxy, Rate Limiter, Embedding Service | Success, Auth Failure, Validation Failure, Upstream Failure, Rate Limit | planned |
| GATEWAY-EMBED-003 | POST | /api/v1/embed/similarity | API Key | Auth Proxy, Rate Limiter, Embedding Service | Success, Auth Failure, Validation Failure, Upstream Failure, Rate Limit | planned |
| GATEWAY-EMBED-004 | GET | /api/v1/embed/config | Public | Embedding Service | Success, Service Failure Mapping | planned |
| GATEWAY-EMBED-005 | POST | /api/v1/embed/config | Admin JWT | Admin JWT Verify, Embedding Service | Admin Success, Non-admin Rejection, Validation Failure | planned |
...
| GATEWAY-DOCS-001 | GET | /api/v1/documents/overview | Public | DB, Vector Store | Success, Fallback on Missing DB | planned |
| GATEWAY-DOCS-002 | GET | /api/v1/documents/preview | Public | DB, Vector Store | Success, Validation Failure, Not Found | planned |
| GATEWAY-DOCS-003 | GET | /api/v1/documents/download-url | Public | Storage URL Resolver | Success, Validation Failure | planned |
...
- Move status from `planned` -> `implemented` -> `passing` as tests land and pass in CI.
