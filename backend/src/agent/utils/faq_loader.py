...
# [utils/faq_loader.py]
#
# Loads the curated markdown FAQ file into the 'curated_content' table.
# This script is designed to be run manually for fast updates to FAQs/Rules
# without re-loading the entire database.
# It assumes it is run from the project's root directory.
import logging
import os
...
def main():
    ...
    supabase_url_auth = os.environ.get("SUPABASE_URL")
    supabase_key_auth = os.environ.get("SUPABASE_KEY")
    if not supabase_url_auth or not supabase_key_auth:
    ...
    # 5. Upload new chunks
    log.info(f"Uploading {len(data_to_upload)} new chunks to '{TABLE_NAME}'...")
