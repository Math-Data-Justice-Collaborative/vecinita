...
def test_render_prod_template_satisfies_contract() -> None:
    env = parse_env_file(REPO_ROOT / "config/.env.prod.render.example")
...
def test_render_staging_template_satisfies_contract() -> None:
    env = parse_env_file(REPO_ROOT / "config/.env.staging.render.example")
    result = validate_shared_render_env(env)
    assert result.errors == []
...
def test_render_templates_key_parity() -> None:
    prod = parse_env_file(REPO_ROOT / "config/.env.prod.render.example")
    staging = parse_env_file(REPO_ROOT / "config/.env.staging.render.example")
...
def test_local_template_contains_core_cross_service_keys() -> None:
    ...
    root_env = parse_env_file(REPO_ROOT / "config/.env.example")
