...
def test_list_catchup_residuals_returns_targets() -> None:
    """TC-341: InternalWriteClient fetches catch-up residual targets from write API."""
    document_id = uuid4()

    def handler(request: httpx.Request) -> httpx.Response:
        assert request.method == "GET"
        assert request.url.path == "/internal/v1/automations/residuals"
        assert request.headers["Authorization"] == "Bearer test-key"
        return httpx.Response(
            HTTPStatus.OK,
            json={
    ...
    residuals = client.list_catchup_residuals()
    assert isinstance(residuals, CatchupResidualListResponse)
