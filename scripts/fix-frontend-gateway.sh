...
FRONTEND_ENV_FILE="./apps/chat-frontend/.env"
FRONTEND_ENV_PROD_FILE="./apps/chat-frontend/.env.production"
...
if [ "$(git -C . status --porcelain | wc -l)" -gt 0 ]; then
  ...
  git add apps/chat-frontend/.env apps/chat-frontend/.env.production 2>/dev/null || true
