...
**Gateway-owned Modal scraper control plane (`MODAL_SCRAPER_PERSIST_VIA_GATEWAY`):** When set to **`1`** / **`true`** on the **Render gateway** (`vecinita-gateway`), the gateway inserts the `scraping_jobs` row using the service’s normal **`DATABASE_URL`** (internal hostname is fine). It then calls Modal `modal_scrape_job_submit` with a **`job_id`** in the payload so Modal **only enqueues** scrape work and does **not** open Postgres for that RPC. Set the **same** flag in Modal secret **`vecinita-scraper-env`** so the scraper service honors the split. For **get / list / cancel** on `/api/v1/modal-jobs/scraper`, the gateway reads and updates Postgres directly and **does not** call the corresponding Modal RPCs while the flag is enabled. Pipeline workers on Modal may still require **`MODAL_DATABASE_URL`** until worker stages are migrated off Postgres (see `specs/001-gateway-live-schema/plan.md`).
