...
Production deployments must not use mock/placeholder/example DSNs for canonical corpus paths; startup/runtime guards fail closed when those markers are detected.
...
Blueprint builds [`modal-apps/scraper`](../../modal-apps/scraper) (`vecinita_scraper`). At startup the process validates config and **fails immediately** if production auth is incomplete (`ConfigError: Missing required auth configuration…`).
...
**Offline pytest** (`make test-schemathesis`, gateway + agent) uses mocked upstreams in CI. The **scraper / data-management** HTTP API also has offline Schemathesis in [`modal-apps/scraper/tests/integration/test_openapi_schemathesis.py`](../../modal-apps/scraper/tests/integration/test_openapi_schemathesis.py) (mocked job control; runs in the scraper package test suite).
