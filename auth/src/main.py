...
class RateLimitState:
    ...
    def record_failed_attempt(self, api_key: str):
        ...
        if self.failed_attempts[api_key] >= self.max_failed_attempts:
            ...
            logger.warning(f"API key blocked after {self.max_failed_attempts} failed attempts: {api_key[:10]}...")
...
async def get_api_key_from_header(authorization: Optional[str] = Header(None)) -> str:
    ...
    if api_key in rate_limit.blocked_keys:
        raise HTTPException(
            status_code=429,
            detail="API key is temporarily blocked due to too many failed attempts"
        )
...
async def get_usage(api_key: str = Depends(get_api_key_from_header)):
    ...
    - Time of last reset
    """
    stats = rate_limit.usage.get(api_key, {
        "tokens_today": 0,
        "requests_today": 0,
