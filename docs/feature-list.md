...
| F83 | Distinct staging environment (DO + Supabase + Modal) | Planned | Cross-cutting (infra) | DO apps/DB, Supabase project, Modal workspace `vecinita-staging`, GH Environments + ruleset | EV-staging-do-supabase; ADR-054 |
...
- **What it does**: Provisions a **true non-prod staging** stack that mirrors production:
  DigitalOcean apps + Managed Postgres, Supabase Auth project, and a **separate Modal
  workspace** (`vecinita-staging`). Restores staging→prod paths and ends operational use of
...
- **Inputs**: Operator tokens (DO, Modal staging workspace, Supabase); GitHub Environments
