...
1. Identify what changed: `git diff` / stated scope — focus on `backend/src/api/`, `packages/openapi-clients/`, `services/data-management-api/`, OpenAPI snapshots, and `frontend/src` (especially `**/services/**`, `**/lib/**`, hooks calling HTTP).
