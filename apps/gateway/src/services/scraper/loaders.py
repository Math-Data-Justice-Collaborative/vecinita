...
class SmartLoader:
    ...
    def load_url(
        self, url: str, failed_log: str | None = None, force_loader: str | None = None
    ) -> tuple[list, str, bool]:
        """
        ...
        try:
            ...
            docs, loader_type, success = self._select_and_load(url, force_loader)
            ...
            if success:
                ...
                try:
                    lengths = [len(getattr(d, "page_content", "") or "") for d in docs]
                    if lengths:
                        min_len = min(lengths)
                        max_len = max(lengths)
                    ...
                    for d in docs[:3]:
                        ...
                        samples.append(meta.get("source") or meta.get("url") or meta.get("title"))
                    if samples:
                        log.info(f"--> Sample sources: {', '.join([str(s) for s in samples if s])}")
    ...
    def _select_and_load(self, url: str, force_loader: str | None) -> tuple[list, str, bool]:
    ...
    def _load_recursive(self, url: str, config: dict) -> tuple[list, str, bool]:
        ...
        try:
            ...
            if len(docs) == 0:
                log.warning("--> No documents found. Waiting 5 seconds and retrying...")
                time.sleep(5)
                try:
                    docs = loader.load()
    ...
    def _load_playwright(self, url: str) -> tuple[list, str, bool]:
        ...
        log.debug(f"--> Waiting {wait_time} seconds for JavaScript content to fully load...")
