...
- **Purpose**: Compact time-series for dashboard charts (M64 / UJ-041).
- **Auth**: Admin JWT required.
- **Query**:
  - `metrics` — comma-separated metric keys (e.g. `faithfulness,retrieval_relevance,answer_relevancy,latency_p95_ms` + custom criterion keys)
  - `since` — ISO8601 lower bound on `started_at` (optional)
  - `until` — ISO8601 upper bound (optional)
  - `run_ids` — comma-separated UUID filter (optional)
  - `limit` — max points (default 100, max 500)
- **Response** `200`:
...
- **Errors**: `400` invalid metric key; `403` viewer.

### GET `/internal/v1/eval/criteria`

- **Purpose**: List built-in + custom evaluation criteria (M64 / UJ-043).
- **Auth**: Admin JWT required.
- **Response** `200`:
...
### POST `/internal/v1/eval/criteria`

- **Purpose**: Create a custom evaluation criterion.
- **Auth**: Admin JWT required.
- **Request**:
...
```json
{
  ...
  "threshold": 0.7,
  "enabled": true,
  "config": { "rubric_prompt": "Rate conciseness 0-1..." }
}
```

- **Response** `201`: created criterion object.
- **Errors**: `409` duplicate `key`; `400` validation; `403` viewer.

### PATCH `/internal/v1/eval/criteria/{id}`

- **Purpose**: Update name, threshold, enabled, or config of a custom criterion (built-in rows: `enabled` + `threshold` only).
- **Auth**: Admin JWT required.
- **Response** `200`: updated criterion.
- **Errors**: `404` unknown id; `403` viewer.
